...
const getAlertsData = (district) => ({
  ...
  past: [
    { id: 3, location: "from Mysore Road", resolved: true, closed: true },
    { id: 4, location: "from Indiranagar", resolved: true, closed: true },
    { id: 5, location: "from J.P. Nagar", resolved: true, closed: true },
  ],
  myAlerts: {
    active: {
      id: 6,
    ...
    history: [
      { id: 7, location: "from Mysore Road", resolved: true, closed: true },
      { id: 8, location: "from Indiranagar", resolved: true, closed: true },
    ],
  },
});
  ...
  return (
    <div className="min-h-screen min-h-dvh bg-background flex flex-col safe-area-top safe-area-bottom">
      <div className="flex-1 flex flex-col px-5 sm:px-6 md:px-8 pt-12 sm:pt-14 pb-24 max-w-lg mx-auto w-full">
        ...
        {/* Title */}
        <div className="mt-6">
          <h1 className="text-4xl sm:text-5xl font-bold text-foreground leading-none">Rapid</h1>
          <h2 className="text-2xl sm:text-3xl text-secondary leading-none mt-1">Response Team</h2>
        </div>

        {/* SOS Alerts Header */}
        <div className="mt-6 flex items-center gap-2">
          <h3 className="text-xl sm:text-2xl font-bold text-foreground">SOS Alerts</h3>
          <span className="w-2 h-2 rounded-full bg-[#E53935]" />
        </div>

        {/* Tab Bar */}
        <div className="mt-4 bg-[#262626] rounded-lg p-1 flex">
          {['current', 'past', 'myAlerts'].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2 px-3 rounded-md text-sm font-semibold tracking-wide transition-all ${
                activeTab === tab
                  ? tab === 'past' ? 'bg-[#424242] text-white' : 'bg-white text-black'
                  : 'text-secondary hover:text-foreground'
              }`}
            >
              {tab === 'myAlerts' ? 'MY ALERTS' : tab.toUpperCase()}
            </button>
          ))}
        </div>
                  ...
                  <div key={alert.id} className="bg-[#202020] border border-[#333333] rounded-lg p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-secondary mt-0.5">{alert.location}</p>
                      </div>
                      <div className="bg-[#333333] rounded-lg px-3 py-1.5 flex items-center gap-1.5 flex-shrink-0">
                        <MapPin className="w-4 h-4 text-foreground" />
                        <span className="text-sm font-semibold text-foreground">{alert.distance}</span>
                      </div>
                    </div>
              ...
              {alerts.past.map((alert) => (
                <div key={alert.id} className="bg-[#1E1E1E] border border-[#2A2A2A] rounded-lg overflow-hidden">
                  <div className="p-4 flex items-center justify-between">
                    <p className="text-base text-secondary">{alert.location}</p>
                  ...
                  <div className="bg-[#2A2A2A] px-4 py-3 flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-foreground tracking-wide uppercase">Closed</span>
                  </div>
                </div>
              ))}
            </>
              ...
              {alerts.myAlerts.active && (
                <div className="bg-[#1C1C1C] border border-[#333333] rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-lg sm:text-xl font-bold text-foreground">{alerts.myAlerts.active.location}</h4>
                    <div className="bg-[#2A2A2A] rounded-lg px-3 py-1.5 flex items-center gap-1.5">
                      <MapPin className="w-4 h-4 text-foreground" />
                      <span className="text-sm font-semibold text-foreground">{alerts.myAlerts.active.distance}</span>
                    </div>
                  </div>
                  <div className="mt-4 flex items-center gap-2">
                    <button onClick={() => toast.info("Viewing attendees...")} className="flex-1 h-11 sm:h-12 bg-[#2A2A2A] rounded-lg flex items-center justify-center gap-2">
                      <Users className="w-5 h-5 text-foreground" />
                      <span className="text-base font-bold text-foreground">{alerts.myAlerts.active.attendingCount} ATTENDING</span>
                    </button>
                    <button onClick={handleChat} className="flex-1 h-11 sm:h-12 bg-white rounded-lg flex items-center justify-center gap-2">
                      ...
                      <span className="text-base font-bold text-black">CHAT</span>
                    </button>
                  </div>
                </div>
              )}
